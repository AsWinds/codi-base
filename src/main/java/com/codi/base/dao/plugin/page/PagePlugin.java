package com.codi.base.dao.plugin.page;
...
public class PagePlugin implements Interceptor {
    ...
    private String dialect = null;// 数据库类型
    ...
    public Object intercept(Invocation ivk) throws Throwable {
        if (ivk.getTarget() instanceof RoutingStatementHandler) {
            ...
            String queryTotalCountString = null;
            Boolean useSimpleSqlForCount = false;
            ...
            if (parameterObject == null) {
                ...
                if (parameterObject instanceof PageView) { // 参数就是Pages实体
                    ...
                    for (Entry entry : (Set<Entry>) parameters.entrySet()) {
                        if (entry.getValue() instanceof PageView) {
                            pageView = (PageView) entry.getValue();
                            if (parameters.containsKey("sqltotalcount")) {
                                queryTotalCountString = parameters.get("sqltotalcount").toString();
                            } else if (parameters.containsKey("simpleSqlForCount")) {
                                useSimpleSqlForCount = true;
                            }
                ...
                setPageParameter(sql, queryTotalCountString, useSimpleSqlForCount, connection, mappedStatement,
                        boundSql, parameterObject, pageView);
     ...
     * @throws SQLException
     */
    private void setPageParameter(String sql, String queryTotalCountString, Boolean useSimpleSqlForCount,
            Connection connection, MappedStatement mappedStatement, BoundSql boundSql, Object parameterObject,
        ...
        String countSql;
        try {
            if (isEmpty(queryTotalCountString)) {
                if (useSimpleSqlForCount)
                    sql = this.shrinkSimpleSqlForCount(sql);

                countSql = "select count(1) from (" + sql + ") tmp_count"; // 记录统计
            } else {
                countSql = queryTotalCountString;
            }
    ...
    private String shrinkSimpleSqlForCount(String sql) {
        int start = sql.indexOf("from", 1);
        String secondPartSql = sql.substring(start);
        return "select 1 " + secondPartSql;
    }
